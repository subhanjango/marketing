<?php namespace App\Http\Controllers;
	...
	class AdminAppUsersController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Products Chosen","name"=>"products_chosen"];
			...
			$this->form[] = ['label'=>'Products Chosen','name'=>'products_chosen','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','datatable'=>'products,title'];
			...
			//$this->form[] = ['label'=>'Address','name'=>'address','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			//$this->form[] = ['label'=>'City','name'=>'city','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			//$this->form[] = ['label'=>'Products Chosen','name'=>'products_chosen','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			# OLD END FORM
	    ...
	    */
		...
		public function addUser(LaravelRequest $request)
		{
			$this->cbInit();

			$message = ['products.required' => 'Please choose a product first from above.' ];
			...
			$request->validate([
				...
				'city' => 'required|min:1|max:15',
				'products' => 'required|array'
			],$message);

			foreach($request->products as $product_id)
			{
				$products_array[] = DB::table('products')->find($product_id)->title;
			}

			$request->merge(['products_chosen' => implode(',',$products_array)]);

			DB::table($this->table)->insert($request->only(['name' , 'email' , 'number' , 'address' , 'city' , 'products_chosen']));

			return response()->json(['success'=>'Your requested has been submitted successfully.']);
		}
